  ...
  if item.type == "detail" and node.category == 'recipe' then
    for _,currentRecipe in pairs(recipes) do
      if currentRecipe.name == item.name then
        item.description = currentRecipe.description or "Receta de pocion"
        item.info = currentRecipe.info or "Desconocido"
        break
      end
    end
  else
    item.description = node.description or "item"
    item.info = node.info or "informacion desconocida"
  end
...
function Item:use(player, thrower)
  ...
  if self.type == "weapon" or self.type == "scroll" then
    assert(self.props.subtype,"Se requiere un subtipo para el arma ("..self.name..")")
