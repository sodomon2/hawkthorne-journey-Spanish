...
function Updater:progress()
  if not self.thread then
    return "Esperando para empezar", 0
  end
  ...
  if self:done() then
    return "Actualizacion finalizada", 100
  end
...
function sparkle.update(version, url, callback)
  ...
  if platform == nil then
    error("La plataforma actual no soporta actualizaciones automaticas")
  end
  ...
  if oldpath == "" then
    error("No puedo encontrar el directorio de la aplicacion")
  end

  pcall(callback, false, "Busqueda de actualizaciones", 0)

  -- Download appcast
  local b, c, h = http.request(url)

  if c >= 400 then
    error("No se puede obtener appcast.json, devuelto HTTP " .. tostring(c))
  end
  ...
  if item == nil then
    pcall(callback, true, "La version actual está actualizada", 100)
    return
  end
  ...
  if download == nil then
    error("No se puede encontrar la descarga en el artículo appcast")
  end
