...
function i18n:initialize(path, locale)
  self.path = path
  self:setLocale("es-ES")
end

function i18n:getLocale(locale)
  local path = self.path .. "/" .. locale .. '.json'
  assert(love.filesystem.exists(path), string.format("La traduccion %q es desconocido", locale))
...
function i18n:translate(id)
  ...
  assert(result, string.format("La identificacio no se encontro la traduccion actual %q  (%q)", id, self.currentLocale))
  return result
end
