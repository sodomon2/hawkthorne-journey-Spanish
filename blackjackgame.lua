...
function state:keypressed(button, player)
  if button == 'JUMP' and self.selected == 'SALIR' then
    Prompt.new("¿Estas seguro de que quieres salir?",
      function(result)
        if result == 'Yes' then
...
function state:draw()
  ...
  if self.dealer_done then
    if self.dealerHand[1].score > 21 then
      love.graphics.print("PECHO", self.dealer_result_pos_x, self.dealer_result_pos_y, 0, 0.5)
    else
    ...
    if self.playerHand[1].score then
      for i=1,self.numOfHands do
        if self.playerHand[i].score > 21 then
          love.graphics.print("PECHO", self.dealer_result_pos_x, self.dealer_result_pos_y +101+(i-1)*14, 0, 0.5)
        else
  ...
  -- print current bet
  love.graphics.print('Apuesta $ ' .. self.currentBet, 361+camera.x, 141+camera.y, 0, 0.5)
