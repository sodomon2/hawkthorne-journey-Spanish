...
function Quest.addQuestItem(quest, player)
  ...
  itemNode.description = "Busqueda de " .. quest.questParent
...
function Quest.giveQuestFail(npc, player, quest)
  local script = "Ya tienes una busqueda '" .. player.quest .. "' a favor de {{red_light}}" .. player.questParent .. "{{white}}!"
  script = quest.giveQuestFail or script
  Dialog.new(script, function()
    npc.prompt = prompt.new("¿Abandonar la búsqueda actual?", function(result)
      if result == 'Yes' then
