...
function Inventory:animation()
  assert(self.animations[self.animState], "Estado " .. self.animState .. " no tiene una animacion correspondiente!")
...
function Inventory:switchPage( direction )
  ...
  assert(nextState, 'Error de cambio de página de inventario')
...
function Inventory:dropItem(item, slotIndex, page)
  ...
  -- Must set the quantity after creating the Node.
    ...
    assert(myNewNode.draw, 'ERROR: ' .. myNewNode.name ..  ' no tiene funcion de sorteo!')
    level:addNode(myNewNode)
    assert(level:hasNode(myNewNode), 'ERROR: La funcion de caida no agrego correctamente ' .. myNewNode.name .. ' al nivel!')--]]
...
function Inventory:addItem(item, sfx, callback)
  ...
  assert(self.pages[pageName], "Mal tipo de articulo! " .. item.type .. " no es una clase de posicion valida.")
...
function Inventory:currentPage()
  assert(self:isOpen(), "El inventario esta cerrado, no se puede obtener la pagina actual cuando el inventario esta cerrado.")
  local page = self.pages[self.currentPageName]
  assert(page, "No he podido encontrar la pagina ".. self.currentPageName)
  return page
end
  ...
  for key,value in pairs( json.decode( saved_inventory ) ) do
    ...
    for key2 , saved_item in pairs( value ) do
      ...
      elseif saved_item.type == Item.types.ITEM_DETAIL then
        ...
        print( "Advertencia: tipo de elemento guardado sin manipular: " .. saved_item.type )
      end
      ...
      if itemNode then
        ...
        print( "Advertencia: elemento guardado desconocido: " .. itemNode.name)
        end
      end
