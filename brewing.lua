...
function state:enter(previous, player, screenshot, cauldronName)
  ...
  self.text = "Las recetas se pueden encontrar a lo largo de todo el juego. Es una buena idea recogerlas. "..
              "Si no sigues las recetas, haras una poción de Dud, que tiene efectos secundarios impredecibles cuando se consume."
...
function state:brew( potion )
  ...
  local message = 'Hiciste un {{red}}'..item.description..'{{white}}!'
...
function state:draw()
  ...
  if self.total == 0 then
    love.graphics.printf("Aun no tienes ninguna receta.", x2 + 16, menu_top + 50, 132, "center")
  else
    ...
    for _,currentRecipe in pairs(potion_recipes) do
      ...
      if currentRecipe.name == recipe.name then
        ...
        if self.total > 1 then
          recipeInstructions = fonts.tasty.new("Presiona {{red}}" .. self.player.controls:getKey('INTERACT') .. "{{white}} o {{red}}" .. self.player.controls:getKey('ATTACK') .. "{{white}} para ver otras recetas.", x2 + 16, menu_top + 100, 132, love.graphics.getFont(), fonts.colors)
...
function state:update(dt)
  assert(type(dt)=="number", "el tiempo de actualizacion (dt) debe ser un numero")
end
