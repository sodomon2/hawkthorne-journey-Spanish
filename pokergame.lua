...
function state:init()
  ...
  self.options = {
    { name = 'SORTEO', action = 'poker_draw' },
    { name = 'OFERTA', action = 'deal_hand' },
    { name = 'APUESTA +', action = function() 
    ...
    { name = 'Salir', action = 'quit', active = true },
  }
  self.selection = 2
...
function state:quit()
  self.prompt = Prompt.new("¿Estas seguro de que quieres salir?", function(result)
    if result == 'Yes' then
      self.card_complete_callback = nil
...
function state:poker_draw()
  ...
  self.card_complete_callback = function()
    ...
    elseif(comp == 1) then
      self.outcome = "El Distribuidor Gana!"
      if self.nakedBet then
        self:strip_lose()
      else
        self.player.money = self.player.money - self.bet
...
function state:strip_poker()
  self.prompt = Prompt.new("¿No tienes dinero, pero tu ropa vale mas o menos. {{yellow}}" .. self.nakedMoney .. " monedas{{white}}. Te gustaría apostarlas?", function(result)
    if result == 'No' then
      Gamestate.switch(self.previous)
    else
      self.nakedBet = true
...
function state:strip_lose()
  self.prompt = Dialog.new("El repartidor gana. Quitate la ropa y vete.", function(result)
...
function state:game_over()
  self.prompt = Dialog.new("Se acabo el juego.", function(result)
    Gamestate.switch(self.previous)
  end )
end
...
function state:draw()
  ...
  love.graphics.print( 'En la mano\n $ ' .. self.player.money, 80+36 + camera.x, 213+33+camera.y, 0, 0.5 )

  if self.nakedBet then
    love.graphics.print('Ropa de apuestas', 315+36+camera.x, 112+33+camera.y, 0, 0.5 )
  else    
    love.graphics.print( 'Apuesta $ ' .. self.bet , 315+36+camera.x, 112+33+camera.y, 0, 0.5 )
  end
...
function evaluate_hand(hand)
  ...
  HIGH_CARD = {value = 0, name = "Carta Alta", format_string = "Carta Alta: %s"}
  PAIR = {value = 1, name = "Par", format_string = "Par de %s"}
  TWO_PAIR = {value = 2, name = "Dos Pares", format_string = "Dos Pares: %s y %s"}
  THREE_OF_A_KIND = {value = 3, name = "Tres de un tipo", format_string = "Tres %s"}
  STRAIGHT = {value = 4, name = "Recto", format_string = "%s Recto alto"}
  FLUSH = {value = 5, name = "Escalera", friendly_name = "Escalera"}
  FULL_HOUSE = {value = 6, name = "Casa Llena", format_string = "%s Lleno de %s"}
  FOUR_OF_A_KIND = {value = 7, name = "Cuatro de un tipo", format_string = "Cuatro %s"}
  STRAIGHT_FLUSH = {value = 8, name = "Escalera de color", format_string = "%s Escalera Alta De Recto"}
  ROYAL_FLUSH = {value = 9, name = "Escalera Real", friendly_name = "Escalera Real"}
