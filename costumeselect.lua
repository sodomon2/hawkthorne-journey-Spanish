...
function state:enter(previous, target)
  ...
  self.chartext = "PRESIONA " .. controls:getKey('JUMP') .. " PARA ELEGIR EL PERSONAJE"
  self.menutext = "PRESIONA " .. controls:getKey('START') .. " PARA VOLVER AL MENU"
  self.costext = "PRESIONA " .. controls:getKey('JUMP') .. " PARA ELEGIR EL DISFRAZ" 
  self.backtext = "PRESIONA " .. controls:getKey('ATTACK') .. " CAMBIAR DE JUGADOR"
end
  ...
  -- Only draw the details on the screen when the background is up
    if not background.slideIn then
      ...
      if self.side == 1 and self.level == 3 then
       love.graphics.printf('Insuficientes amigos', 0, 23, window.width, 'center')
      else
        local name = ""
        if self:character() then
      ...
      love.graphics.print("INSUFICIENTES", x, y + 5, 0, 0.5, 0.5, 12, -6)
      love.graphics.print(  "AMIGOS"   , x, y + 5, 0, 0.5, 0.5, -12, -32)
      love.graphics.setColor(255, 255, 255, 255)
    end
