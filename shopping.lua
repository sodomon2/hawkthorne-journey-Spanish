...
function state:init()
  ...
  self.items = {}
  self.purchaseOptions = {"COMPRAR", "VENDER"} 
...
function state:enter(previous, player, screenshot, supplierName)
  ...
  assert(self.supplier,"proveedor con el nombre de "..self.supplierName.." no tiene contenido")
  assert(utils.propcount(self.supplier)>0, "el proveedor debe tener al menos una categoria")

  self.selectText = "PRESIONA " .. player.controls:getKey('JUMP') .. " PARA SELECCIONAR"
  self.backText = "PRESIONA " .. player.controls:getKey('ATTACK') .. " PARA VOLVER"
  self.tooltipText = "PRESIONA " .. player.controls:getKey('INTERACT') .. " PARA ABRIR EL TOOLTIP"
  ...
  for category,stock in pairs(self.supplier) do
    for _,info in pairs(stock) do
      ...
      elseif category=="keys" then
        ...
        error("Categoria no soportada: "..category)
      end
      info["item"] = item
    end
  end
...
function state:purchaseWindowKeypressed( button )
  ...
  if button == "JUMP" then
    if self.purchaseOptions[self.purchaseSelection] == "COMPRAR" then
      self:buySelectedItem()
      sound.playSfx('confirm')
    elseif self.purchaseOptions[self.purchaseSelection] == "VENDER" then
...
function state:buySelectedItem()
  ...
  if self.player.money < cost*self.buyAmount then
    self.message = "No tienes suficiente dinero para hacer esta compra."
    self.window = "messageWindow"

  elseif amount <= 0 then
    self.message = "Este articulo esta agotado."
    self.window = "messageWindow"
    ...
    for i = 1,self.buyAmount do
      ...
      if itemInfo.action then
        ...
        if not self.player.inventory:addItem(itemCopy) then
          if i == 1 then
            self.message = "Compra sin exito. Su inventario esta lleno."
          else
            itemInfo[2] = itemInfo[2] - (i-1)
            self.player.money = self.player.money - cost*(i-1)
            self.message = "Solo compraste " .. i-1 .. " de estos artículos porque tu inventario esta lleno."
          end
          self.window = "messageWindow"
          return
        end
...
function state:sellSelectedItem()
  ...
  if iamount <= 0 or not playerItem then
    self.message = "No tienes nada de esto para vender."
    self.window = "messageWindow"
  ...
  elseif playerItem.name == name then
    ...
    self.message = "Ha vendido este articulo con exito."
    self.window = "messageWindow"
  end
...
function state:draw()
  ...
  elseif self.window == "itemsWindow" then
    ...
    for i, itemInfo in pairs(self.items) do
      ...
      if ( i >= self.itemsWindowLeft and i <= self.itemsWindowLeft + ROW) then
        ...
        love.graphics.print(cost .. " monedas", xcorner + 15 + 32*visI, ycorner + 45, 0, 0.5, 0.5 )
  ...
  elseif self.window == "purchaseWindow" then
    ...
    if self.purchaseOptions[self.purchaseSelection] == "COMPARA" then
    love.graphics.print(amount .. " en existencia", xcorner + 13, ycorner + 44, 0, 0.5, 0.5 )
    elseif self.purchaseOptions[self.purchaseSelection] == "VENDER" then
    love.graphics.print( iamount .. " en el inventario", xcorner + 8, ycorner + 44, 0, 0.5, 0.5 )
    end
    ...
    love.graphics.print("Comprar  " .. self.buyAmount, xcorner + 63, ycorner + 25)
    love.graphics.print("ender " .. self.sellAmount, xcorner + 63, ycorner + 40)
