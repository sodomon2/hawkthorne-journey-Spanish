...
menu:onSelect(function()
  ...
  state.statusText = "PRESIONA NUEVA TECLA" end)
...
function state:draw()
  ...
  local back = controls:getKey("START") .. ": VOLVER AL MENU"
  local howto = controls:getKey("ATTACK") .. " O " .. controls:getKey("JUMP") .. ": REASIGNAR CONTROL"
...
function state:remapKey(key)
  ...
  if not controls:newAction(key, button) then
    self.statusText = "LA TECLA YA ESTA EN USO"
  else
    ...
    if button == "START" and key ~= "escape" then key = key .. " o ESCAPE" end
    if button == "JUMP" and key ~= "return" then key = key .. " o ENTER" end
